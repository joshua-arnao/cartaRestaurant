...
export async function getTablesApi(token) {
  try {
    const url = `${BASE_API_URL}/api/tables/`;
    const params = {
      headers: {
...
export async function updateTableApi(id, data, token) {
  try {
    const url = `${BASE_API_URL}/api/tables/${id}/`;
    const params = {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
...
export async function getTableApi(idTable) {
  try {
    const url = `${BASE_API_URL}/api/tables/${idTable}/`;
...
export async function getTableByNumberApi(numberTable) {
  try {
    ...
    const url = `${BASE_API_URL}/api/tables/?${tableFilter}`;
    const response = await fetch(url);
    const result = await response.json();
    return result;
  } catch (error) {
