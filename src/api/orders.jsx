import { BASE_API_URL, ORDER_STATUS } from "../utils/constans";
...
export async function checkDeliveredOrderApi(id) {
  try {
    const url = `${BASE_API_URL}/api/orders/${id}/`;
    const params = {
      method: "PATCH",
      headers: {
...
export async function addOrderToTableApi(idTable, idProduct) {
  try {
    const url = `${BASE_API_URL}/api/orders/`;
    const params = {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function addPaymentToOrderApi(idOrder, idPayment) {
  try {
    const url = `${BASE_API_URL}/api/orders/${idOrder}/`;
    const params = {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
...
export async function getOrdersByPaymentApi(idPayment) {
  try {
    ...
    const url = `${BASE_API_URL}/api/orders/?${paymentFilter}`;
    const response = await fetch(url);
    const result = await response.json();
    return result;
  } catch (error) {
